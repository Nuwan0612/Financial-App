...
const InvestmentsPage = () => {
  ...
  useEffect(() => {
    accountsApi.getAll()
      .then(res => setAccounts(res.data.filter(a => a.type === "Investment")))
  ...
  return (
    <div className="p-6 mx-auto space-y-6" style={{ maxWidth: "1200px" }}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Investment Accounts</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Total balance across all investments:{" "}
            <span className="font-semibold text-foreground">{fmt(totalBalance)}</span>
          </p>
        </div>
        <Button size="sm" onClick={() => setShowAdd(true)}>
          <Plus className="h-4 w-4 mr-1" /> Add Investment Account
        </Button>
      </div>
      {/* ----------------------------- */}
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-border py-16 text-center">
          <p className="text-muted-foreground text-sm">No investment accounts found.</p>
          <p className="text-xs text-muted-foreground mt-1">
            Go to Settings → Accounts to add one.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
